...
class TmpldController(CementBaseController):
    class Meta:
        ...
        arguments = [
            ...
            (
                ["-s", "--strict"],
                dict(
                    help="Raise an exception if a variable is not defined",
                    action="store_true",
                    default=False,
                ),
            ),
        ...
        if name == "kube":
            try:
                kube = pyrkube.KubeAPIClient(
                    config["environment"], config["namespace"], config["domain"]
                )
    ...
    def _get_extensions(self):
        config = self.app.config.get_section_dict("tmpld")
        extensions = {name: self._get_ext(name, config) for name in config["exts"]}
...
class TmpldApp(CementApp):
    class Meta:
        ...
        config_defaults = {
            ...
            "log.kwlogging": dict(level=os.getenv("TMPLD_LOG_LEVEL", "WARNING")),
        }
